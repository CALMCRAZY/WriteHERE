...
class RegularDummyNode(AbstractNode):   
    ...
    def plan(self, agent, memory, *args, **kwargs):
        ...
        # Enhanced logging after plan2graph
        logger.info(f"Node {self.nid} (Goal: {self.task_info.get('goal', 'N/A')[:50]}...): plan2graph executed. Number of sub-tasks in inner_graph: {len(self.inner_graph.topological_task_queue)}")
        if self.raw_plan and isinstance(self.raw_plan, list):
            logger.info(f"Raw plan for Node {self.nid} contained {len(self.raw_plan)} items.")
        elif self.raw_plan:
            logger.info(f"Raw plan for Node {self.nid} was present but not a list (type: {type(self.raw_plan)}).")
        else:
            logger.warning(f"Raw plan for Node {self.nid} was empty or None.")
        return result
